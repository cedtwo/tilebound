...
impl AxisMask {
    ...
    /// Create an `AxisMask` with only the bit of the given axis and [`Endpoint`] set.
    pub fn from_end<A: Axis>(end: Endpoint) -> Self {
        let mask = match (A::VALUE, end) {
            (AxisX::VALUE, Endpoint::LOW) => AxisMask::LEFT.0,
            (AxisX::VALUE, Endpoint::UPP) => AxisMask::RIGHT.0,
            (AxisY::VALUE, Endpoint::LOW) => AxisMask::TOP.0,
            (AxisY::VALUE, Endpoint::UPP) => AxisMask::BOTTOM.0,
            _ => unreachable!(),
        };
        AxisMask(mask)
    }

    /// Returns `true` if the bit of the given axis and [`Endpoint`] is set.
    pub fn end_is_set<A: Axis>(&self, end: Endpoint) -> bool {
        match (A::VALUE, end) {
            (AxisX::VALUE, Endpoint::LOW) => self.0 & AxisMask::LEFT.0 != 0,
            (AxisX::VALUE, Endpoint::UPP) => self.0 & AxisMask::RIGHT.0 != 0,
            (AxisY::VALUE, Endpoint::LOW) => self.0 & AxisMask::TOP.0 != 0,
            (AxisY::VALUE, Endpoint::UPP) => self.0 & AxisMask::BOTTOM.0 != 0,
            _ => unreachable!(),
        }
    }

    /// Returns `true` if either bit of the given [`Endpoint`] is set.
    pub fn any_end_is_set(&self, end: Endpoint) -> bool {
        match end {
            Endpoint::Lower => self.0 & AxisMask::TOP_LEFT.0 != 0,
            Endpoint::Upper => self.0 & AxisMask::BOTTOM_RIGHT.0 != 0,
        }
    }
    ...
    /// Set the bit at the given axis [`Endpoint`].
    pub fn set_end<A: Axis>(&mut self, end: Endpoint) {
        self.0 |= AxisMask::from_end::<A>(end).0;
    }

    /// Clear the bit of the given axis and `Sign`.
    pub fn clear_end<A: Axis>(&mut self, sign: Endpoint) {
    ...
    /// Return the first set [`Endpoint`] (using LSB ordering) for the given axis.
    pub fn first_end<A: Axis>(&self) -> Option<Endpoint> {
        self.isolate_axis::<A>().any_first_end()
    }

    /// Return the first set [`Endpoint`] (using LSB ordering) for either axis.
    pub fn any_first_end(&self) -> Option<Endpoint> {
        let mask = *self;
    ...
    /// Return the first set [`Endpoint`] (using LSB ordering) for either axis. Assumes at least one
    /// bit is set, panicing in debug and returning [`Endpoint::Upper`] where no bit is set. This
    /// should only be used where certain (at least) one bit is set. Prefer
    /// [`AxisMask::any_first_end`] if unsure.
    pub fn any_first_end_unchecked(&self) -> Endpoint {
...
mod tests {
    ...
    fn clear_end() {
        ...
        mask_br.clear_end::<AxisX>(Endpoint::LOW);
        mask_br.clear_end::<AxisY>(Endpoint::LOW);
        let mut mask_tl = AxisMask::ALL;
        mask_tl.clear_end::<AxisX>(Endpoint::UPP);
        mask_tl.clear_end::<AxisY>(Endpoint::UPP);
